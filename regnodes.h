...
#define END_tb_pb             	0	/* 0000 End of program. */
#define END_tb_p8             	1	/* 0x01  */
#define END_t8_pb             	2	/* 0x02  */
#define END_t8_p8             	3	/* 0x03  */
#define SUCCEED               	1	/* 0x01 Return from a subroutine, basically. */
#define SUCCEED_tb_pb         	4	/* 0x04 Return from a subroutine, basically. */
...
#define GPOS_tb_pb            	28	/* 0x1c Matches where last m//g left off. */
...
#define BOUND_tb_pb           	32	/* 0x20 Like BOUNDA for non-utf8, otherwise like BOUNDU */
...
#define BOUNDL_tb_pb          	36	/* 0x24 Like BOUND/BOUNDU, but \w and \W are defined by current locale */
...
#define BOUNDU_tb_pb          	40	/* 0x28 Match "" at any boundary of a given type using /u rules. */
...
#define BOUNDA_tb_pb          	44	/* 0x2c Match "" at any boundary between \w\W or \W\w, where \w is [_a-zA-Z0-9] */
...
#define NBOUND_tb_pb          	48	/* 0x30 Like NBOUNDA for non-utf8, otherwise like BOUNDU */
...
#define NBOUNDL_tb_pb         	52	/* 0x34 Like NBOUND/NBOUNDU, but \w and \W are defined by current locale */
...
#define NBOUNDU_tb_pb         	56	/* 0x38 Match "" at any non-boundary of a given type using using /u rules. */
...
#define NBOUNDA_tb_pb         	60	/* 0x3c Match "" betweeen any \w\w or \W\W, where \w is [_a-zA-Z0-9] */
...
#define REG_ANY_tb_pb         	64	/* 0x40 Match any one character (except newline). */
...
#define SANY_tb_pb            	68	/* 0x44 Match any one character. */
...
#define ANYOF_tb_pb           	72	/* 0x48 Match character in (or not in) this class, single char match only */
...
#define ANYOFD_tb_pb          	76	/* 0x4c Like ANYOF, but /d is in effect */
...
#define ANYOFL_tb_pb          	80	/* 0x50 Like ANYOF, but /l is in effect */
...
#define ANYOFPOSIXL_tb_pb     	84	/* 0x54 Like ANYOFL, but matches [[:posix:]] classes */
...
#define ANYOFH_tb_pb          	88	/* 0x58 Like ANYOF, but only has "High" matches, none in the bitmap; the flags field contains the lowest matchable UTF-8 start byte */
...
#define ANYOFHb_tb_pb         	92	/* 0x5c Like ANYOFH, but all matches share the same UTF-8 start byte, given in the flags field */
...
#define ANYOFHr_tb_pb         	96	/* 0x60 Like ANYOFH, but the flags field contains packed bounds for all matchable UTF-8 start bytes. */
...
#define ANYOFHs_tb_pb         	100	/* 0x64 Like ANYOFHb, but has a string field that gives the leading matchable UTF-8 bytes; flags field is len */
...
#define ANYOFR_tb_pb          	104	/* 0x68 Matches any character in the range given by its packed args: upper 12 bits is the max delta from the base lower 20; the flags field contains the lowest matchable UTF-8 start byte */
...
#define ANYOFRb_tb_pb         	108	/* 0x6c Like ANYOFR, but all matches share the same UTF-8 start byte, given in the flags field */
...
#define ANYOFM_tb_pb          	112	/* 0x70 Like ANYOF, but matches an invariant byte as determined by the mask and arg */
...
#define NANYOFM_tb_pb         	116	/* 0x74 complement of ANYOFM */
...
#define POSIXD_tb_pb          	120	/* 0x78 Some [[:class:]] under /d; the FLAGS field gives which one */
...
#define POSIXL_tb_pb          	124	/* 0x7c Some [[:class:]] under /l; the FLAGS field gives which one */
...
#define POSIXU_tb_pb          	128	/* 0x80 Some [[:class:]] under /u; the FLAGS field gives which one */
...
#define POSIXA_tb_pb          	132	/* 0x84 Some [[:class:]] under /a; the FLAGS field gives which one */
...
#define NPOSIXD_tb_pb         	136	/* 0x88 complement of POSIXD, [[:^class:]] */
...
#define NPOSIXL_tb_pb         	140	/* 0x8c complement of POSIXL, [[:^class:]] */
...
#define NPOSIXU_tb_pb         	144	/* 0x90 complement of POSIXU, [[:^class:]] */
...
#define NPOSIXA_tb_pb         	148	/* 0x94 complement of POSIXA, [[:^class:]] */
...
#define CLUMP_tb_pb           	152	/* 0x98 Match any extended grapheme cluster sequence */
...
#define BRANCH_tb_pb          	156	/* 0x9c Match this alternative, or the next... */
...
#define EXACT_tb_pb           	160	/* 0xa0 Match this string (flags field is the length). */
...
#define LEXACT_tb_pb          	164	/* 0xa4 Match this long string (preceded by length; flags unused). */
...
#define EXACTL_tb_pb          	168	/* 0xa8 Like EXACT, but /l is in effect (used so locale-related warnings can be checked for) */
...
#define EXACTF_tb_pb          	172	/* 0xac Like EXACT, but match using /id rules; (string not UTF-8, ASCII folded; non-ASCII not) */
...
#define EXACTFL_tb_pb         	176	/* 0xb0 Like EXACT, but match using /il rules; (string not likely to be folded) */
...
#define EXACTFU_tb_pb         	180	/* 0xb4 Like EXACT, but match using /iu rules; (string folded) */
...
#define EXACTFAA_tb_pb        	184	/* 0xb8 Like EXACT, but match using /iaa rules; (string folded except in non-UTF8 patterns: MICRO, SHARP S; folded length <= unfolded) */
...
#define EXACTFUP_tb_pb        	188	/* 0xbc Like EXACT, but match using /iu rules; (string not UTF-8, folded except MICRO, SHARP S: hence Problematic) */
...
#define EXACTFLU8_tb_pb       	192	/* 0xc0 Like EXACTFU, but use /il, UTF-8, (string is folded, and everything in it is above 255 */
...
#define EXACTFAA_NO_TRIE_tb_pb	196	/* 0xc4 Like EXACT, but match using /iaa rules (string not UTF-8, not guaranteed to be folded, not currently trie-able) */
...
#define EXACT_REQ8_tb_pb      	200	/* 0xc8 Like EXACT, but only UTF-8 encoded targets can match */
...
#define LEXACT_REQ8_tb_pb     	204	/* 0xcc Like LEXACT, but only UTF-8 encoded targets can match */
...
#define EXACTFU_REQ8_tb_pb    	208	/* 0xd0 Like EXACTFU, but only UTF-8 encoded targets can match */
...
#define EXACTFU_S_EDGE_tb_pb  	212	/* 0xd4 /di rules, but nothing in it precludes /ui, except begins and/or ends with [Ss]; (string not UTF-8; compile-time only) */
...
#define LNBREAK_tb_pb         	216	/* 0xd8 generic newline pattern */
...
#define TRIE_tb_pb            	220	/* 0xdc Match many EXACT(F[ALU]?)? at once. flags==type */
...
#define TRIEC_tb_pb           	224	/* 0xe0 Same as TRIE, but with embedded charclass data */
...
#define AHOCORASICK_tb_pb     	228	/* 0xe4 Aho Corasick stclass. flags==type */
...
#define AHOCORASICKC_tb_pb    	232	/* 0xe8 Same as AHOCORASICK, but with embedded charclass data */
...
#define NOTHING_tb_pb         	236	/* 0xec Match empty string. */
...
#define TAIL_tb_pb            	240	/* 0xf0 Match empty string. Can jump here from outside. */
...
#define STAR_tb_pb            	244	/* 0xf4 Match this (simple) thing 0 or more times. */
...
#define PLUS_tb_pb            	248	/* 0xf8 Match this (simple) thing 1 or more times. */
...
#define CURLY_tb_pb           	252	/* 0xfc Match this simple thing {n,m} times. */
...
#define CURLYN_tb_pb          	256	/* 0x100 Capture next-after-this simple thing */
...
#define CURLYM_tb_pb          	260	/* 0x104 Capture this medium-complex thing {n,m} times. */
...
#define CURLYX_tb_pb          	264	/* 0x108 Match this complex thing {n,m} times. */
...
#define WHILEM_tb_pb          	268	/* 0x10c Do curly processing and see if rest matches. */
...
#define OPEN_tb_pb            	272	/* 0x110 Mark this point in input as start of #n. */
...
#define CLOSE_tb_pb           	276	/* 0x114 Close corresponding OPEN of #n. */
...
#define SROPEN_tb_pb          	280	/* 0x118 Same as OPEN, but for script run */
...
#define SRCLOSE_tb_pb         	284	/* 0x11c Close preceding SROPEN */
...
#define REF_tb_pb             	288	/* 0x120 Match some already matched string */
...
#define REFF_tb_pb            	292	/* 0x124 Match already matched string, using /di rules. */
...
#define REFFL_tb_pb           	296	/* 0x128 Match already matched string, using /li rules. */
...
#define REFFU_tb_pb           	300	/* 0x12c Match already matched string, usng /ui. */
...
#define REFFA_tb_pb           	304	/* 0x130 Match already matched string, using /aai rules. */
...
#define REFN_tb_pb            	308	/* 0x134 Match some already matched string */
...
#define REFFN_tb_pb           	312	/* 0x138 Match already matched string, using /di rules. */
...
#define REFFLN_tb_pb          	316	/* 0x13c Match already matched string, using /li rules. */
...
#define REFFUN_tb_pb          	320	/* 0x140 Match already matched string, using /ui rules. */
...
#define REFFAN_tb_pb          	324	/* 0x144 Match already matched string, using /aai rules. */
...
#define LONGJMP_tb_pb         	328	/* 0x148 Jump far away. */
...
#define BRANCHJ_tb_pb         	332	/* 0x14c BRANCH with long offset. */
...
#define IFMATCH_tb_pb         	336	/* 0x150 Succeeds if the following matches; non-zero flags "f", next_off "o" means lookbehind assertion starting "f..(f-o)" characters before current */
...
#define UNLESSM_tb_pb         	340	/* 0x154 Fails if the following matches; non-zero flags "f", next_off "o" means lookbehind assertion starting "f..(f-o)" characters before current */
...
#define SUSPEND_tb_pb         	344	/* 0x158 "Independent" sub-RE. */
...
#define IFTHEN_tb_pb          	348	/* 0x15c Switch, should be preceded by switcher. */
...
#define GROUPP_tb_pb          	352	/* 0x160 Whether the group matched. */
...
#define EVAL_tb_pb            	356	/* 0x164 Execute some Perl code. */
...
#define MINMOD_tb_pb          	360	/* 0x168 Next operator is not greedy. */
...
#define LOGICAL_tb_pb         	364	/* 0x16c Next opcode should set the flag only. */
...
#define RENUM_tb_pb           	368	/* 0x170 Group with independently numbered parens. */
...
#define GOSUB_tb_pb           	372	/* 0x174 recurse to paren arg1 at (signed) ofs arg2 */
...
#define GROUPPN_tb_pb         	376	/* 0x178 Whether the group matched. */
...
#define INSUBP_tb_pb          	380	/* 0x17c Whether we are in a specific recurse. */
...
#define DEFINEP_tb_pb         	384	/* 0x180 Never execute directly. */
...
#define ENDLIKE_tb_pb         	388	/* 0x184 Used only for the type field of verbs */
...
#define OPFAIL_tb_pb          	392	/* 0x188 Same as (?!), but with verb arg */
...
#define ACCEPT_tb_pb          	396	/* 0x18c Accepts the current matched string, with verbar */
...
#define VERB_tb_pb            	400	/* 0x190 Used only for the type field of verbs */
...
#define PRUNE_tb_pb           	404	/* 0x194 Pattern fails at this startpoint if no-backtracking through this */
...
#define MARKPOINT_tb_pb       	408	/* 0x198 Push the current location for rollback by cut. */
...
#define SKIP_tb_pb            	412	/* 0x19c On failure skip forward (to the mark) before retrying */
...
#define COMMIT_tb_pb          	416	/* 0x1a0 Pattern fails outright if backtracking through this */
...
#define CUTGROUP_tb_pb        	420	/* 0x1a4 On failure go to the next alternation in the group */
...
#define KEEPS_tb_pb           	424	/* 0x1a8 $& begins here. */
...
#define OPTIMIZED_tb_pb       	428	/* 0x1ac Placeholder for dump. */
...
#define PSEUDO_tb_pb          	432	/* 0x1b0 Pseudo opcode for internal use. */
...
#define REGEX_SET_tb_pb       	436	/* 0x1b4 Regex set, temporary node used in pre-optimization compilation */
...
#define TRIE_next_tb_pb       	440	/* 0x1b8 state for TRIE */
...
#define TRIE_next_fail_tb_pb  	444	/* 0x1bc state for TRIE */
...
#define EVAL_B_tb_pb          	448	/* 0x1c0 state for EVAL */
...
#define EVAL_B_fail_tb_pb     	452	/* 0x1c4 state for EVAL */
...
#define EVAL_postponed_AB_tb_pb	456	/* 0x1c8 state for EVAL */
...
#define EVAL_postponed_AB_fail_tb_pb	460	/* 0x1cc state for EVAL */
...
#define CURLYX_end_tb_pb      	464	/* 0x1d0 state for CURLYX */
...
#define CURLYX_end_fail_tb_pb 	468	/* 0x1d4 state for CURLYX */
...
#define WHILEM_A_pre_tb_pb    	472	/* 0x1d8 state for WHILEM */
...
#define WHILEM_A_pre_fail_tb_pb	476	/* 0x1dc state for WHILEM */
...
#define WHILEM_A_min_tb_pb    	480	/* 0x1e0 state for WHILEM */
...
#define WHILEM_A_min_fail_tb_pb	484	/* 0x1e4 state for WHILEM */
...
#define WHILEM_A_max_tb_pb    	488	/* 0x1e8 state for WHILEM */
...
#define WHILEM_A_max_fail_tb_pb	492	/* 0x1ec state for WHILEM */
...
#define WHILEM_B_min_tb_pb    	496	/* 0x1f0 state for WHILEM */
...
#define WHILEM_B_min_fail_tb_pb	500	/* 0x1f4 state for WHILEM */
...
#define WHILEM_B_max_tb_pb    	504	/* 0x1f8 state for WHILEM */
...
#define WHILEM_B_max_fail_tb_pb	508	/* 0x1fc state for WHILEM */
...
#define BRANCH_next_tb_pb     	512	/* 0x200 state for BRANCH */
...
#define BRANCH_next_fail_tb_pb	516	/* 0x204 state for BRANCH */
...
#define CURLYM_A_tb_pb        	520	/* 0x208 state for CURLYM */
...
#define CURLYM_A_fail_tb_pb   	524	/* 0x20c state for CURLYM */
...
#define CURLYM_B_tb_pb        	528	/* 0x210 state for CURLYM */
...
#define CURLYM_B_fail_tb_pb   	532	/* 0x214 state for CURLYM */
...
#define IFMATCH_A_tb_pb       	536	/* 0x218 state for IFMATCH */
...
#define IFMATCH_A_fail_tb_pb  	540	/* 0x21c state for IFMATCH */
...
#define CURLY_B_min_tb_pb     	544	/* 0x220 state for CURLY */
...
#define CURLY_B_min_fail_tb_pb	548	/* 0x224 state for CURLY */
...
#define CURLY_B_max_tb_pb     	552	/* 0x228 state for CURLY */
...
#define CURLY_B_max_fail_tb_pb	556	/* 0x22c state for CURLY */
...
#define COMMIT_next_tb_pb     	560	/* 0x230 state for COMMIT */
...
#define COMMIT_next_fail_tb_pb	564	/* 0x234 state for COMMIT */
...
#define MARKPOINT_next_tb_pb  	568	/* 0x238 state for MARKPOINT */
...
#define MARKPOINT_next_fail_tb_pb	572	/* 0x23c state for MARKPOINT */
...
#define SKIP_next_tb_pb       	576	/* 0x240 state for SKIP */
...
#define SKIP_next_fail_tb_pb  	580	/* 0x244 state for SKIP */
...
#define CUTGROUP_next_tb_pb   	584	/* 0x248 state for CUTGROUP */
...
#define CUTGROUP_next_fail_tb_pb	588	/* 0x24c state for CUTGROUP */
...
#define KEEPS_next_tb_pb      	592	/* 0x250 state for KEEPS */
...
#define KEEPS_next_fail_tb_pb 	596	/* 0x254 state for KEEPS */
